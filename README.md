...
├── .github/
│   └── workflows/
│       ├── terraform-ci.yaml        # fmt·validate CI
│       └── terraform-cd.yaml        # 수동 apply workflow
├── bootstrap/                       # S3 + DynamoDB remote state 최초 생성
│   ├── main.tf
│   ├── variables.tf
│   └── terraform.tfvars.example
├── environments/
│   └── dev/                         # 개발 환경의 모듈 조합과 변수
│       ├── main.tf
│       ├── variables.tf
│       └── terraform.tfvars.example
├── modules/
...
`bootstrap/`은 원격 상태 인프라를 별도로 준비하고, `environments/dev/`는 11개 모듈을 조합해 개발 환경을 구성합니다.
...
- `terraform-ci`: `main` push와 pull request에서 backend 없이 초기화한 뒤 `terraform fmt -check`와 `terraform validate` 실행
- `terraform-cd`: 실 backend, untracked `terraform.tfvars`, AWS Secrets를 설정한 뒤 확인 입력과 함께 수동 실행하는 apply workflow
- 두 workflow 모두 Terraform `1.5.7` 사용
- AWS 자격증명은 수동 apply workflow에서만 GitHub Actions Secrets로 주입
